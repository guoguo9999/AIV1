...
augpipe_specs = {
    ...
    'bgc_nohue':       dict(xflip=1, rotate90=1, xint=1, scale=1, rotate=1, aniso=1, xfrac=1, brightness=1, contrast=1, lumaflip=0, hue=0, saturation=1),
    'bgc':       dict(xflip=1, rotate90=1, xint=1, scale=1, rotate=1, aniso=1, xfrac=1, brightness=1, contrast=1, lumaflip=1, hue=1, saturation=1),
    'bgc_nocolor':       dict(xflip=1, rotate90=1, xint=1, scale=1, rotate=1, aniso=1, xfrac=1, brightness=1, contrast=1, lumaflip=0, hue=0, saturation=1),

    'bgc_norotate':       dict(xflip=1, rotate90=0, xint=1, scale=1, rotate=0, aniso=1, xfrac=1, brightness=1, contrast=1, lumaflip=1, hue=1, saturation=1),
...
def process_hyperparams(cfg: DictConfig):
    ...
    print('train set', args.training_set_kwargs)
    try:
    ...
    if 'r1_gamma' in cfg.model.loss_kwargs:
        ...
        assert isinstance(r1_gamma, float) or isinstance(r1_gamma, int)
        if not r1_gamma >= 0:
    ...
    print('resume', c.resume, flush=True)
    ...
    if c.resume is None or c.resume == 'noresume':
        ...
        print(len(ckpts), run_dir, flush=True)
        if len(ckpts) > 0:

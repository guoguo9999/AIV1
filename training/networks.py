...
from src.training.motion import MotionMappingNetwork, BSplineMotionMappingNetwork
...
class SynthesisNetwork3v(torch.nn.Module):
    def __init__(self,
        ...
        img_channels,                   # 通道数
        channel_base        = 32768,    # 通道的基准乘数
        channel_max         = 512,      # 单层最大通道数
        num_layers          = 14,       # 总层数
        num_critical        = 2,        # 末端临界采样层数
        first_cutoff        = 2,        # 第一层截止频率
        first_stopband      = 2**2.1,   # 第一层最小阻带频率
        last_stopband_rel   = 2**0.3,   # 最后一层最小阻带频率
        margin_size         = 10,       # 图像外部的额外像素边距
        output_scale        = 0.25,     # 输出图像的缩放因子
        num_fp16_res        = 4,        # 使用FP16精度的最高分辨率层数
        cfg                 = {},       # 参数字典
        **layer_kwargs,                 # 合成层的额外参数
    ):
...
class Generator(torch.nn.Module):
    def __init__(self,
        c_dim,                      # 条件标签的维度
        w_dim,                      # 中间潜在空间的维度
        img_resolution,             # 生成图像的分辨率
        img_channels,               # 生成图像的颜色通道数
        mapping_kwargs      = {},   # 配置映射网络的参数
        synthesis_kwargs    = {},   # 配置合成网络的参数
        cfg                 = {},   # 包含配置信息的字典
    ):
        super().__init__()
...
class DiscriminatorBlock(torch.nn.Module):
    def __init__(self,
        ...
        out_channels,                       # 输出通道数
        resolution,                         # 输入分辨率
        img_channels,                       # 输入图像的颜色通道数
        first_layer_idx,                    # 第一层的索引
        architecture        = 'resnet',     # 采用的架构类型
        activation          = 'lrelu',      # 激活函数累型
        resample_filter     = [1,3,3,1],    # 低通滤波器
        conv_clamp          = None,         # 对卷积层的输出进行裁剪的阈值
        use_fp16            = False,        # 是否使用fp16
        fp16_channels_last  = False,        # Use channels-last memory format with FP16?
        freeze_layers       = 0,            # 训练过程中冻结的层数
        cfg                 = {},           # 配置字典
    ):
        assert architecture in ['orig', 'skip', 'resnet']
        super().__init__()
...
class Discriminator(torch.nn.Module):
    def __init__(self,
        c_dim,                          #条件标签的维度
        img_resolution,                 # 输入图像的分辨率
        img_channels,                   # 输入图像的通道数
        architecture        = 'resnet', # 架构类型
        channel_base        = 32768,    #通道的基准乘数
        channel_max         = 512,      # 任意一层允许的最大通道数
        num_fp16_res        = 0,        # 对最高的N个分辨率层使用fp16计算
        conv_clamp          = None,     # 对卷积层输出裁剪的阈值
        cmap_dim            = None,     # 映射后的条件标签的维度
        block_kwargs        = {},       # 判别器块的额外参数
        mapping_kwargs      = {},       # 映射网络的额外参数
        epilogue_kwargs     = {},       # 判别器尾声的额外参数
        cfg                 = {},       # 额外的配置字典
    ):
        super().__init__()
